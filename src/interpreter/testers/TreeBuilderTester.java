...
/**
 * Tester to fix bugs in TreeBuilder class and confirm correct output
 * 
 * @author maddiebriere
 *
 */
public class TreeBuilderTester {
	...
	public static void main(String[] args) {
		...
		String tree = "to tree [ :length :angle :depth :scale ] [ ifelse equal? :depth 1 "
				+ "[ fd :length bk :length ] [ fd :length lt :angle tree product :length :scale "
				+ ":angle difference :depth 1 :scale rt :angle rt :angle tree product :length :scale "
				+ ":angle difference :depth 1 :scale lt :angle bk :length ] ]";

		TreeBuilder build = new TreeBuilder(spiral, new InstructionClassifier("English"), new InstructionData());
	 ...
	 * 
	 * @param headNodes
	 *            The nodes built by TreeBuilder
	 * @param level
	...
	/**
	 * Print of nodes using Instruction Node methods instead, prints the entire
	 * text starting from this node
	 * 
	 * @param headNodes
	 *            The nodes built by TreeBuilder
	 * @param level
